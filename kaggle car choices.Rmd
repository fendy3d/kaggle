...
trainmlogit_train <- subset(trainmlogit, spl == TRUE) # 'long' subsetted data 
trainmlogit_test <- subset(trainmlogit, spl == FALSE) # 'long' subsetted data
...
model7 <- mlogit(Choice ~ CC+GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price | segment + educ + miles + age + Urb + ppark + night + gender, data = traindata)
summary(model7)
...
modelX <- mlogit(Choice ~ CC+GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU + Price -1| income + segment+ educ+ Urb+ miles + age + ppark+ region, data = trainmlogit_trainmlogit)
summary(modelX)
AIC(modelX)
...
sum(predmlogitX[1,])
N <- nrow(predmlogitX)
logloss <- -(sum(log(predmlogitX)*trainmlogit_test[,95:98]))/N
logloss




modelY <- mlogit(Choice ~ CC+GN+NS+BU+FA+LD+BZ+FC+FP+RP+PP+KA+SC+TS+NV+MA+LB+AF+HU + Price -1| income + segment+ educ+ Urb+ miles + age + ppark+ region, data = trainmlogit_mlogit)
predmodelY <- predict(modelY, newdata = testmlogit_mlogit)
head(predmodelY,20)
write.csv(predmodelY, "bestmlogit.csv")
```
...
testNN <- test[,4:83]
set.seed(100)
# modelNN <- train(Choice~., data=trainNN, method='nnet',trControl=trainControl(method='cv'))
modelNN <- neuralnet(Choice~CC1+GN1+NS1+BU1+LD1+BZ1+FC1+PP1+KA1+SC1+TS1+NV1+MA1+LB1+AF1+HU1+Price1+
                            ...
                            CC4+GN4+NS4+BU4+LD4+BZ4+FC4+PP4+KA4+SC4+TS4+NV4+MA4+LB4+AF4+HU4+Price4,data=trainNN, hidden=1)
modelNN
...
train_test2 <- subset(train,spl == FALSE)
...
summary(model1)
AIC(model1) #16632.11, LL: -8296.1, logloss: 1.312007 (with Price as num), 1.231658 (with Price as int)
...
modelY <- mlogit(Choice ~ CC+GN+NS+BU+LD+BZ+FC+RP+FP+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price | segment + educ + miles +region + Urb + ppark + night + age + gender + year, data = train_train) #1.183551, 1.179093(with FA,RP,FP, with age,gender,year), 1.185234(without FA,RP,FP, without age,gender,year), 1.180286(with FA,RP,FP, without age,gender,year), (without FA, with age,gender,year)

modelZ <- mlogit(Choice ~ CC+GN+NS+BU+LD+BZ+FC+PP+KA+SC+TS+NV+MA+LB+AF+HU+Price+RP+FP+FA-1 | segment + educ + miles + region + Urb + ppark + night + age + gender + year , rpar = c(CC="n", GN = "n", NS = "n", BU = "n", LD = "n", BZ = "n", FC = "n", PP = "n", KA = "n", SC = "n", TS = "n", NV = "n", MA = "n", LB = "n", AF = "n", HU = "n", FA = "n", FP = "n", RP = "n"), panel = TRUE, data = train_train) #1.26298

predmlogitX <- predict(modelZ, newdata = train_test)
dim(predmlogitX)
sum(predmlogitX[1,])
N <- nrow(predmlogitX)
predmlogit_log <- log(predmlogitX)
logloss <- -(sum(predmlogit_log*train_test2[,95:98]))/N
logloss
...
rf1 <- randomForest(Class ~ ., data = traindatanew, importance = TRUE)
...
information.gain(Choice ~ ., data = trainoftrain)
...
```{r}
library(e1071)
svm1 <- svm(Choice ~., data = trainoftrainnew) #Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : contrasts can be applied only to factors with 2 or more levels
# got zeros in section 4
```

>>>>>>> b7575d0253ba4f3dad2bc98b73523f0277ca0d10
